...
class VolumeWidget(QWidget):
    ...
    def _update_slices(self) -> None:
        ...
        if not self.show_slices_cb.isChecked():
            if self.x_actor:
                self.x_actor.SetVisibility(False)
            if self.y_actor:
                self.y_actor.SetVisibility(False)
            if self.z_actor:
                self.z_actor.SetVisibility(False)
            self.plotter.render()
            return
        ...
        self.x_actor = self.plotter.add_mesh(
            x_slice, cmap="seismic", name="x_slice", show_scalar_bar=False
        )
        self.y_actor = self.plotter.add_mesh(
            y_slice, cmap="seismic", name="y_slice", show_scalar_bar=False
        )
        self.z_actor = self.plotter.add_mesh(
            z_slice, cmap="seismic", name="z_slice", show_scalar_bar=False
        )
    ...
    def _create_volume(self) -> None:
        ...
        self.vol_actor = self.plotter.add_volume(
            self.mesh, cmap="seismic", opacity=opacity, name="volume"
        )
    ...
    def _create_isosurface(self) -> None:
        ...
        try:
            ...
            self.iso_actor = self.plotter.add_mesh(
                iso_mesh, color="red", name="isosurface", opacity=0.5
            )
