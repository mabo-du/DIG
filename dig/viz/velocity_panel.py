...
class VelocityPanel(QtWidgets.QWidget):
    ...
    def __init__(self, radargram_widget: Any = None, parent: Optional[QtWidgets.QWidget] = None):
        ...
        self.temp_hyperbola_item = pg.PlotDataItem(
            pen=pg.mkPen("y", width=2, style=QtCore.Qt.PenStyle.DashLine)
        )
        self.scatter_item = pg.ScatterPlotItem(
            size=8, pen=pg.mkPen(None), brush=pg.mkBrush(255, 0, 0, 120)
        )
            ...
            if dt > 0 and dx > 0:
                ...
                if t_sq_diff > 0:
                    ...
                    self.hyperbolas.append(
                        {"apex_trace": apex_trace, "apex_time": apex_time, "velocity": v}
                    )
    ...
    def _update_hyperbolas(self) -> None:
        ...
        for hyp in self.hyperbolas:
            ...
            times = self._calc_hyperbola(
                traces, hyp["apex_trace"], hyp["apex_time"], hyp["velocity"]
            )
    ...
    def _calc_hyperbola(
        self, traces: np.ndarray, apex_trace: float, apex_time: float, v: float
    ) -> np.ndarray:
    ...
    def export_json(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Export Velocity Model", "", "JSON Files (*.json)"
        )
        if path:
