...
if __name__ == "__main__":
    ...
    med_profiles = [
        Profile(name=f"test_{i}", data=np.random.randn(1000, 1024).astype(np.float32), sample_interval_ns=0.1, trace_spacing_m=0.05)
        for i in range(10)
    ]
    ...
    large_profiles = [
        Profile(name=f"test_{i}", data=np.random.randn(5000, 2048).astype(np.float32), sample_interval_ns=0.1, trace_spacing_m=0.05)
        for i in range(10)
    ]
